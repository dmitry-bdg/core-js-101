...
function getIntervalString(a, b, isStartIncluded, isEndIncluded) {
  return (`${(isStartIncluded ? '[' : '(').toString() + Math.min(a, b)}, ${
    Math.max(a, b)}${(isEndIncluded ? ']' : ')').toString()}`);
}
...
function isCreditCardNumber(ccn) {
  return [...ccn.toString()].reverse().map((x) => parseInt(x, 10))
...
function getDigitalRoot(num) {
  ...
  if (sumArray.toString().split('').length < 2) {
    return sumArray;
  }
  const result = sumArray.toString().split('');
  let sum = 0;
